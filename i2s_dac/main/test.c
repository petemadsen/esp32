/* ADC1 Example
   ...
   https://github.com/stevenvdr/esp-adc-bug
*/
#include "sdkconfig.h"
...
void dac_init()
{
    ...
    i2s_set_sample_rates(I2S_NUM, 2000); //set sample rates
    ...
    for (uint i = 0; i < 256; i++)
    {
        ...
        if (i % 2 ) {
            samples1[i/2] =  ((char) sin_float) << 8;// ((i % 8) << 8) + ((i % 8) << 24);
        }
        else {
            samples1[i/2] =  ((char) sin_float) << 24;// ((i % 8) << 8) + ((i % 8) << 24);
        }
    }
    ...
    for (uint i = 0; i < 256; i++)
    {
        ...
        if (i % 2 ) {
            samples2[i/2] =  ((char) sin_float) << 8;// ((i % 8) << 8) + ((i % 8) << 24);
        }
        else {
            samples2[i/2] =  ((char) sin_float) << 24;// ((i % 8) << 8) + ((i % 8) << 24);
        }
    }
    // Write data to the buffer. This is timing dependent, so be careful with
    // changing the configuration
    i2s_write_bytes(I2S_NUM, (char*) &samples1, 256, portMAX_DELAY);
    i2s_write_bytes(I2S_NUM, (char*) &samples1, 256, portMAX_DELAY);
    i2s_write_bytes(I2S_NUM, (char*) &samples2, 256, portMAX_DELAY);
    i2s_write_bytes(I2S_NUM, (char*) &samples2, 256, portMAX_DELAY);
